...
# This script creates multi-dimensional tensors (at least 3D) from Pore-C, one tensor per chromosome.
...
# The maximum cardinality to include in the tensor
max_cardinality = 3     # this determines the number of dimensions in the tensor
...
tensor_directory = os.path.join(output_directory, 'tensors')
...
def create_tensors(cardinality_data, bin_size, chrom):
    ...
    dim = math.ceil(chr_length / bin_size)
    ...
    # iterate over count_dict and remove any entries with a count less than min_freq
    for key, value in list(count_dict.items()):
        if value < min_freq:
            del count_dict[key]

    dims = [dim]*max_cardinality
    data = np.array(list(count_dict.values()), dtype=np.float64)
    idxs = np.array(list(count_dict.keys()), dtype=np.int64)
    
    # Create the TensorFox sparse tensor
    T = [data, idxs, dims]

    print("created tensor for " + chrom)
    print("dimensions: ", T[2])
...
# path to directory containing gz files of Pore-C reads by chromosome
...
for gz_file in gz_files:
    ...
    df.loc[:, 'bin'] = (df['midpoint'] / bin_size).apply(np.ceil).astype(int)
